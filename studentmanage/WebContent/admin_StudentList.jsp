...
<div class="x-body">
  <form class="layui-form x-center" name="frm" action="StudentList" method="get" style="width:85%">
    <div class="layui-form-pane">
      <div class="layui-form-item">
        ...
        <div class="layui-input-inline" style="width:100px">
          <input type="text" name="classId"  placeholder="班级" autocomplete="off" class="layui-input" value="${classId}">
        </div>
        <label class="layui-form-label">学院号码</label>
        <div class="layui-input-inline" style="width:150px;border:1px;">
		  ...
		  <select style="display:block;" name="acdemyId" class="layui-input">
			<option value="">请选择</option>
				<c:forEach items="${acdemyList}"	var="acde">		
						<option value="${acde.acdemyId}" <c:if test="${acde.acdemyId==acdemyId}">${"selected"}</c:if> >[${acde.acdemyId}] ${acde.acdemyName}</option>
			    </c:forEach>
		  </select>
        </div>  
  ...
  <div class="tools">
    <ul class="toolbar">
      ...
      <li onclick="user_management_add('添加用户','admin_StudentAdd.jsp','600','500')"><span><img src="images/t01.png" /></span>添加</li>
      </ul>
    ...
    <tbody>
      <c:forEach items="${list}"	var="bean">		
					<tr>
						...
						<td>${bean.classId}</td>								
						<td><a href="studentimage.jsp?id=${bean.studentId }&img=${bean.image }"><img src=  <c:choose> <c:when test="${bean.image==null}">"images\TX.jpg"</c:when> <c:otherwise>'image/${bean.image }'</c:otherwise></c:choose> style="width:40px;;heigth:40px;"></a></td>
						<td>${bean.acdemyId}</td>
						<td>${bean.tel}</td>
						<td>${bean.sex}</td>
						<td>${bean.nation}</td>
						...
						<td>
						<ul class="toolbar">
     					 <li onclick="user_management_edit('用户编辑','admin_StudentEdit?id=${bean.studentId}','600','500')"><span><img src="images/t02.png" /></span>编辑</li>
      					 <li onclick="queren(${bean.studentId })"><span><img src="images/t03.png" /></span>删除</li>
    					</ul></td>
					</tr>
			</c:forEach>
    </tbody>
...
<script>
	...
	function queren(i){
		var se=confirm("是否删除");
		if (se==true){
			window.location.href='admin_StudentDelete?id='+i;		
		}
	}
