...
<script type="text/javascript">
	...
	function queren(i){
		var se=confirm("是否删除");
		if (se==true){
			window.location.href='acdemyDelete?id='+i;		
		}
	}
</script>
...
<div class="x-body">
  <form class="layui-form x-center" name="frm" method="get" action="admin_CourseList" style="width:85%">
        ...
        <div class="layui-input-inline" style="width:150px;border:1px;">
		  ...
		  <select style="display:block;" name="courseId" class="layui-input">
			<option value="">请选择</option>
				<c:forEach items="${allcourse}"	var="allcourseies">		
						<option value="${allcourseies.courseId}" <c:if test="${allcourseies.courseId==courseId}">${"selected"}</c:if> >[${allcourseies.courseId}] ${allcourseies.courseName}</option>
			    </c:forEach>
		  </select>
        </div>  
        <label class="layui-form-label">课程分数</label>
        <div class="layui-input-inline" style="width:100px">
          <input type="text" name="courseCredits"  placeholder="分数" autocomplete="off" class="layui-input" value="${courseCredits }">
        </div>
  ...
  <div class="tools">
    <ul class="toolbar">
      ...
      <li onclick="user_management_add('添加用户','admin_CourseAdd.jsp','600','500')"><span><img src="images/t01.png" /></span>添加</li>
    </ul>
    ...
    <tbody>
      <c:forEach items="${list}"	var="bean">		
					<tr>
						...
						<td>${bean.courseWay}</td>
						<td>${bean.courseCredit}</td>
						<td>
						<ul class="toolbar">
      					 <li onclick="user_management_edit('用户编辑','admin_CourseEdit?courseId=${bean.courseId}','600','500')"><span><img src="images/t02.png" /></span>修改</li>
      					 <li onclick="queren(${bean.courseId })"><span><img src="images/t03.png" /></span>删除</li>
    					</ul></td>
					</tr>
			</c:forEach>
    </tbody>
...
function queren(i){
	var se=confirm("是否删除");
	if (se==true){
		window.location.href='admin_CourseDelete?courseId='+i;		
	}
}
